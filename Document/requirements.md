...
| F-GRP-5 | 视频卡**可拖拽**到其他分组（HTML5 DnD） | ✅ |
| F-GRP-6 | 分组头可折叠 / 展开 | ✅ |
...
| F-LIVE-7 | 算法真实接入时，替换 `state::spawn_scene_state_ticker` 即可 | ⏳ 接口已留 |
...
| 高 | mock 与真实数据一致性 | 浏览器 mock 每次返回默认数据，新增 / 修改不能稳定复现；建议 mock 也持久化或显式标注为只读演示 |
