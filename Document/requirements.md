...
| 编号 | 需求 | 状态 |
| --- | --- | --- |
| F-SET-1 | 主菜单提供「基础设置」「检测配置」「通知配置」三个独立入口 | ✅ |
| F-SET-2 | 基础设置内以 tab 分组展示账号与安全、数据存储 | ✅ |
| F-SET-3 | 检测配置内以 tab 分组展示算法配置、ROI 标定 | ✅ |
| F-SET-4 | 通知配置内以 tab 分组展示通知渠道配置 | ✅ |
| F-SET-5 | 修改登录密码（当前密码 + 新密码 + 确认） | ✅ |
| F-SET-6 | 显示数据存储目录 | ✅ |
...
| 编号 | 需求 | 状态 |
| --- | --- | --- |
| F-NOTIFY-1 | 主菜单提供「通知配置」入口，通知渠道配置在页面内以 tab 组织 | ✅ 已接入 |
...
| F-NOTIFY-14 | 支持分组域检测通知门控：分组开启域检测后，保留单路报警展示，但外部触发通知只在整组启用通道全部报警时发送 | ✅ 已实现 |
